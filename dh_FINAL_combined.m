function dh_FINAL_combined (ipparameter)
...
month_AUT=["Jul", "Aug","Sep","Oct"]; %for northern H
...
DH_ICE=DH_ICE(I);
    ...
    for k = 1:length(t_ice)
        ...
        med_ice(k)=mean(dh_time(I)); %Median of values of a moimnth
    end
...
%     [b,stats] = robustfit(x, y);
    ...
    plot(x1,y1,'k+','MarkerFace','black','LineWidth',2,'MarkerSize',1);hold on %icesat
    plot(x2,new_y2,'bx-','MarkerFace','blue','LineWidth',2, 'MarkerSize',1) %Cryosat
    plot(xx,yy,'g-','LineWidth',2) %seasonal part
    %% try
%     tt=[t_ice;t_cs2];
%     yyyy=b(1)+tt*b(2);
