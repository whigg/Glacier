function dh_FINAL_combined_ICELAND14(ipparameter)
...
%         I= find(dh_time>mean_dh_time-2*std_dh_time & dh_time< mean_dh_time+2*std_dh_time);
        ...
        med_ice(k)=median(dh_time)-22; %Median of values of a moimnth
    end
    ...
    for k = 1:length(t_cs2)
        ...
        dh_time=(DH_CS2(ind));
%         mean_dh_time=median(dh_time);
%         std_dh_time=iqr(dh_time);
%         I= find(dh_time>mean_dh_time-2*std_dh_time & dh_time< mean_dh_time+2*std_dh_time);
         I2=find(abs(dh_time) < 150);
        dh_time=dh_time(I2);
        %med_cs2(k)=mean(dh_time(I));
        med_cs2(k)=median(dh_time)-22;
    end
...
%     [b,stats] = robustfit(x, y);
    ...
    % new_y2=new_y2+10;
     %% 
    filename_plot1='ICELAND.tif'; %filename same as whhat was read .tif
