function find_dh_Cryosat_3(ipparameter)
...
time2=b(:,3);
elev2=b(:,4);
[lat1_utm, lon1_utm]=wgs2utm(lat1,lon1);
[lat2_utm, lon2_utm]=wgs2utm(lat2,lon2);

[lat1, sortIndex] = sort(lat1);
lon1 = lon1(sortIndex);
time=time(sortIndex);
...
for j=1:length(lat2)
    ...
    sum_a=[sum_a;a];
    if a > 1000 && a<=2000   %taking nearby pixels with a definite characteristic
        dh= elev1(i)-elev2(j);
        if abs(dh) < 30
            p_dh=[p_dh;dh];
